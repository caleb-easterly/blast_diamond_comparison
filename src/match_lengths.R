...
query_lengths_df <- read.delim("../rudney_data734ws/seqlengths.tab", header=FALSE, col.names="QueryLength")
query_lengths_df$var <- "query"
query_lengths_df$run <- "query"

blast1_df <- read.delim("out/blast1.tab", header=FALSE, col.names=colNames)
blast1_df$run <- "blast"
blast1_df$var <- "E<1e-3"

diamond1_df <- read.delim("out/diamond1.tab", header=FALSE, col.names=colNames)
...
diamond6_df <- read.delim("out/diamond6.tab", header=FALSE, col.names=colNames)
diamond6_df$run <- "diamond"
diamond6_df$var <- "E<2e5"

blast5_df <- read.delim("out/blast5.tab", header=FALSE, col.names=colNames)
blast5_df$run <- "blast"
blast5_df$var <- "split"

diamond10_df <- read.delim("out/diamond10.tab", header=FALSE, col.names=colNames)
diamond10_df$run <- "diamond"
diamond10_df$var <- "split"
